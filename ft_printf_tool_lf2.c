...
/*   Updated: 2019/02/04 17:10:57 by mwunsch          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libft.h"

static void			ft_conversion_lf_tool_rounding(char **str, int x)
{
	int				i;
...
static void			ft_conversion_lf_tool_precision(char **str)
{
	int				x;
	char			*tmp;
...
void				ft_conversion_lf_tool_getstr(char **str, long double i)
{
	...
	((tmp2 = ft_itoa(i)) == NULL) ? exit(-1) : 0;
	((*str = ft_strcat_free(&tmp, &tmp2)) == NULL) ? exit(-1) : 0;
	if (g_flags.precision == -1)
		g_flags.precision = 6;
	ft_conversion_lf_tool_rounding(str, ft_strlen(*str));
	ft_conversion_lf_tool_precision(str);
}
